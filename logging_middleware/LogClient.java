...
/**
 * Reusable Telemetry Log Client executing asynchronous POST requests to the remote evaluation server.
 */
public class LogClient implements LoggerService {
    ...
    public void log(String stack, String level, String packageName, String message) {
        ...
        // Print formatted log output to standard out
        ...
        // Async API Post payload transmitting to evaluation service
        try {
            ...
            httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    .thenAccept(response -> {
                        if (response.statusCode() >= 300) {
                            System.err.printf("[LogClient WARNING] Log server returned HTTP %d: %s%n", 
                                    response.statusCode(), response.body());
                        }
                    })
                    .exceptionally(ex -> {
                        System.err.println("[LogClient ERROR] Telemetry delivery failed: " + ex.getMessage());
                        return null;
                    });

        } catch (Exception e) {
            System.err.println("[LogClient ERROR] Exception occurred initiating post: " + e.getMessage());
        }
    }
